...
@testset "TumorConfigurations" begin
    @testset "MetaData" begin
        ...
        @test eltype(MetaData(Int64).genotypes) == Int64
        @test MetaData(MD).npops == [MD.npop]
        @test_throws ArgumentError MetaData([1,2],[10,20,30])
        @test sum(MetaData([1,2,3],[10,20,30]).npops) == 60
        ...
        @test_throws ArgumentError M[1, :notafield] = "TEST"
        @test (M[3, :genotype] = "3+") == "3+" && M[3].genotype == "3+"
        @test M[3, :genotype] == "3+"
        @test M[g="3+", :genotype] == "3+"
        @test M[:genotype, g="3+"] == "3+"
        @test_throws ArgumentError push!(M, "0")
        push!(M, "P5")
        @test M[g="P5"].npop == 0
    end
    @testset "TumorConfigurations" begin
        NLconf = nolattice_state()
        @test NLconf.meta.npops == [1]
        push!(NLconf, 2)
        @test NLconf.meta.npops == [1,0]
        for l in [LineLattice, HexagonalLattice, CubicLattice]
            conf = single_center(l, 9; g1=1, g2=2)
            lat = conf.lattice
            dim = dimension(lat)
            ...
            conf[fill(9, dim)...] = 0
            @test sum(conf.meta.npops) == length(lat)-1
            @test_broken conf[:] .= 1
            ##
            for f in [i//9 for i in 1:9]
                conf = half_space(l, 9; f=f, g1=1, g2=2)
                @test conf.meta.npops == Int.(length(conf.lattice).*[1-f, f])
            end
        end
    end
end
